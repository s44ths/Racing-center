...
<div id="page">
	...
	<div id="content">
        <?php
            ...
            $get_req_list = 'SELECT * FROM Request WHERE r_id = :race_id AND req_id IN (SELECT req_id FROM fin_verdict WHERE Verdict = 1) ORDER BY req_id DESC';
			$requests = $racing_center->prepare($get_req_list);
			$requests->execute(['race_id' => $_POST['race_id']]);
			...
			while($row = $requests->fetch(PDO::FETCH_OBJ))
			{
			  ...
			  echo '<div class="post" style="background: url(images/img04.jpg) no-repeat;">
                        ...
                        <div class="longmeta">
                            Статус: <span style="color: green">Одобрена</span>';
			...
			if(!$check) 
			   echo 'На это соревнование нет заявок';
        ?>
